...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    opponent_score //= 10
    points = 3 * abs(opponent_score % 10 - player_score % 10)
    if points > 1:
        return points
    else:
        return 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    x = strategy(score=0, opponent_score=0)
    for score in range(0, goal + 1):
        for opponent_score in range(0, goal + 1):
            if strategy(score=score, opponent_score=opponent_score) != x:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for num_dice in range(1, 11):
        current_score = average_dice(num_dice, dice)
